...
2. Parse the JSON `{router, modelWeight}` and report with a status chip: `🟢 Router ON` or `⚪ Router OFF`, followed by the weight and what it means, e.g. "🟢 Router ON · weight cost — mechanical and search prompts get haiku-delegation directives." Then a one-line matrix visual inside a fenced code block showing which classes inject under the active weight, e.g. for cost: `mechanical ✅  search ✅  complex —`.
