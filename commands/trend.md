...
3. Render per tool. The visual block is MANDATORY: one fenced code block per tool with language `diff` (colors rows: `+` green, `-` red). Tool name + run count as a heading line above the block, then one row per metric: `<metric>  <sparkline>  latest <value>  (<+/- delta> vs previous)`.
   - Row prefix is DIRECTION-AWARE. Lower is better for: `mechanicalMsgs`, `reallocatableTokens`, `reclaimableTokens`, `disable`, `findings`, `wasteTokens`, `compactions`. For those, a negative delta gets `+` (green, improving) and a positive delta gets `-` (red, worsening). All other metrics (`servers`, `plugins`, `totalCalls`, `topOutputTokens`) and zero deltas stay neutral (space prefix). Omit the delta clause when deltas is null.
   - Example:
       ```diff
