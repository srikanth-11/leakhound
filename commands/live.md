...
2. Otherwise, setup:
   ...
   b. Show the exact settings.json snippet with the real resolved path:
      ```json
      "statusLine": { "type": "command", "command": "node \"<resolved path>\\leakhound-statusline.js\"" }
      ```
   c. Ask whether they want you to add it to `~/.claude/settings.json` for them or paste it themselves. Only edit the file with their explicit yes, merging carefully (never clobber other keys).
